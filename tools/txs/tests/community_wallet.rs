...
// TODO: Community wallet transfers are using donor_voice initialization as a assertion
// A migrated community wallet from v5 > v6 will only have cumulative deposits and communitywallet structs
...
async fn new_community_wallet_cant_transfer() -> Result<(), anyhow::Error> {
    ...
    cli_transfer.run()
    .await
    .expect("CLI could not transfer funds to account");
...
async fn remove_community_wallet_admin()  -> Result<(), anyhow::Error> {
    ...
    //TODO: this should be 4 when we are progressing epochs
    assert_eq!(no_of_signers_after_second_proposal, 3, "The number of signers should be 4");
