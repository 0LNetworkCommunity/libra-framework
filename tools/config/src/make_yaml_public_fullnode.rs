...
pub async fn download_genesis(home_dir: Option<PathBuf>) -> anyhow::Result<()> {
    let bytes = reqwest::get(
        "https://raw.githubusercontent.com/0LNetworkCommunity/epoch-archive-mainnet/main/upgrades/v7/genesis.blob",
    )
    .await?
    .bytes()
    .await?;
...
pub async fn get_genesis_waypoint(home_dir: Option<PathBuf>) -> anyhow::Result<Waypoint> {
    let wp_string = reqwest::get(
        "https://raw.githubusercontent.com/0LNetworkCommunity/epoch-archive-mainnet/main/upgrades/v7/waypoint.txt",
    )
    .await?
    .text()
    .await?;
