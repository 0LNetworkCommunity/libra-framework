...
use libra_types::{exports::ValidCryptoMaterialStringExt, legacy_types::{block::VDFProof, app_cfg::AppCfg}};
...
async fn tower_cli_e2e() -> anyhow::Result<()>{
  ...
  //  let proof: VDFProof = serde_json::json!({
  //     "height": 0,
  //     "elapsed_secs": 42552,
  ...
  //   let alice_app_cfg =  AppCfg::init_app_configs(alice.child_0_owner.auth_key, alice.child_0_owner.account, Some(d.path().into()), None, None);

  //   let alice_sender = Sender::from()
