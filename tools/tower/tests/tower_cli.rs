...
async fn tower_cli_e2e() {
    ...
    let p = next_proof::get_next_proof_params_from_local(&app_cfg).expect("could not find a proof locally");
    ...
    let (_total_height, submitted_in_epoch) = backlog::get_remote_tower_height(&app_cfg).await.expect("could not get remote height");
