...
async fn can_vote() {
    ...
    let txn = alice_account.sign_with_transaction_builder(built_tx);
    let res = public_info.client().submit_and_wait(&txn).await.expect("could not send tx");
  

}

#[tokio::test]
async fn cant_vote_twice() {
    ...
    let built_tx = public_info
      ...
      .payload(vote_payload.clone());
    let txn = bob_account.sign_with_transaction_builder(built_tx);
    assert!(public_info.client().submit_and_wait(&txn).await.is_err(), "should not be able to vote twice");
