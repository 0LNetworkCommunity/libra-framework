...
module diem_framework::storage_gas {
    ...
    public(friend) fun set_config(diem_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_diem_framework(diem_framework);
        *borrow_global_mut<StorageGasConfig>(@diem_framework) = config;
    }
    ...
    public fun initialize(diem_framework: &signer) {
        ...
        move_to(diem_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@diem_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(diem_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@diem_framework);
        let gas = borrow_global_mut<StorageGas>(@diem_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@diem_framework);
