...
module diem_framework::object {
    ...
    public(friend) fun address_to_object<T: key>(object: address): Object<T> {
    ...
    public(friend) fun is_object(object: address): bool {
        exists<ObjectCore>(object)
    }

    /// Derives an object address from source material: sha3_256([creator address | seed | 0xFE]).
    public(friend) fun create_object_address(source: &address, seed: vector<u8>): address {
    ...
    public(friend) fun create_user_derived_object_address(source: address, derive_from: address): address {
        let bytes = bcs::to_bytes(&source);
    ...
    public(friend) fun create_guid_object_address(source: address, creation_num: u64): address {
    ...
    public(friend) fun object_address<T: key>(object: &Object<T>): address {
        object.inner
    }

    /// Convert Object<X> to Object<Y>.
    public(friend) fun convert<X: key, Y: key>(object: Object<X>): Object<Y> {
        address_to_object<Y>(object.inner)
    }

    /// Create a new named object and return the ConstructorRef. Named objects can be queried globally
    /// by knowing the user generated seed used to create them. Named objects cannot be deleted.
    public(friend) fun create_named_object(creator: &signer, seed: vector<u8>): ConstructorRef {
    ...
    public(friend) fun create_object_from_account(creator: &signer): ConstructorRef {
    ...
    public(friend) fun create_object_from_object(creator: &signer): ConstructorRef acquires ObjectCore {
    ...
    public(friend) fun generate_delete_ref(ref: &ConstructorRef): DeleteRef {
    ...
    public(friend) fun generate_extend_ref(ref: &ConstructorRef): ExtendRef {
        ExtendRef { self: ref.self }
    }

    /// Generates the TransferRef, which can be used to manage object transfers.
    public(friend) fun generate_transfer_ref(ref: &ConstructorRef): TransferRef {
    ...
    public(friend) fun generate_derive_ref(ref: &ConstructorRef): DeriveRef {
        DeriveRef { self: ref.self }
    }

    /// Create a signer for the ConstructorRef
    public(friend) fun generate_signer(ref: &ConstructorRef): signer {
        create_signer(ref.self)
    }

    /// Returns the address associated with the constructor
    public(friend) fun address_from_constructor_ref(ref: &ConstructorRef): address {
        ref.self
    }

    /// Returns an Object<T> from within a DeleteRef
    public(friend) fun object_from_constructor_ref<T: key>(ref: &ConstructorRef): Object<T> {
        address_to_object<T>(ref.self)
    }

    /// Returns whether or not the ConstructorRef can be used to create DeleteRef
    public(friend) fun can_generate_delete_ref(ref: &ConstructorRef): bool {
        ref.can_delete
    }
    ...
    public(friend) fun create_guid(object: &signer): guid::GUID acquires ObjectCore {
    ...
    public(friend) fun new_event_handle<T: drop + store>(
        object: &signer,
    ...
    /// Returns the address associated with the constructor
    public(friend) fun address_from_delete_ref(ref: &DeleteRef): address {
        ref.self
    }

    /// Returns an Object<T> from within a DeleteRef.
    public(friend) fun object_from_delete_ref<T: key>(ref: &DeleteRef): Object<T> {
        address_to_object<T>(ref.self)
    }

    /// Removes from the specified Object from global storage.
    public(friend) fun delete(ref: DeleteRef) acquires ObjectCore {
    ...
    public(friend) fun generate_signer_for_extending(ref: &ExtendRef): signer {
        create_signer(ref.self)
    }

    /// Returns an address from within a ExtendRef.
    public(friend) fun address_from_extend_ref(ref: &ExtendRef): address {
        ref.self
    }
    ...
    public(friend) fun disable_ungated_transfer(ref: &TransferRef) acquires ObjectCore {
        let object = borrow_global_mut<ObjectCore>(ref.self);
        object.allow_ungated_transfer = false;
    }

    /// Enable direct transfer.
    public(friend) fun enable_ungated_transfer(ref: &TransferRef) acquires ObjectCore {
        let object = borrow_global_mut<ObjectCore>(ref.self);
    ...
    public(friend) fun generate_linear_transfer_ref(ref: &TransferRef): LinearTransferRef acquires ObjectCore {
    ...
    public(friend) fun transfer_with_ref(ref: LinearTransferRef, to: address) acquires ObjectCore {
        let object = borrow_global_mut<ObjectCore>(ref.self);
        assert!(
    ...
    /// hierarchy.
    public(friend) fun transfer_raw(
        owner: &signer,
        object: address,
        to: address,
    ) acquires ObjectCore {
    ...
    public(friend) fun ungated_transfer_allowed<T: key>(object: Object<T>): bool acquires ObjectCore {
        assert!(
    ...
    public(friend) fun owner<T: key>(object: Object<T>): address acquires ObjectCore {
        assert!(
            exists<ObjectCore>(object.inner),
            error::not_found(EOBJECT_DOES_NOT_EXIST),
        );
    ...
    public(friend) fun is_owner<T: key>(object: Object<T>, owner: address): bool acquires ObjectCore {
        owner(object) == owner
    }

    /// Return true if the provided address has indirect or direct ownership of the provided object.
    public(friend) fun owns<T: key>(object: Object<T>, owner: address): bool acquires ObjectCore {
