spec diem_framework::transaction_validation {
    ...
    spec schema PrologueCommonAbortsIf {
        ...
        aborts_if !(txn_sequence_number == global<Account>(transaction_sender).sequence_number);
        aborts_if !exists<CoinStore<GasCoin>>(gas_payer);
        aborts_if !(global<CoinStore<GasCoin>>(gas_payer).coin.value >= max_transaction_fee);
    }
    ...
    spec epilogue(
        ...
        let aptos_addr = type_info::type_of<GasCoin>().account_address;
        // Bindings for `burn_fee` verification.
        let apt_addr = type_info::type_of<GasCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<GasCoin>>(apt_addr).supply;
        ...
        //             ( // `exists<CoinStore<GasCoin>>(addr)` checked above.
        ...
        aborts_if {
            ...
            // Sufficiency of funds is checked above.
            // !exists<AptosCoinCapabilities>(@diem_framework) ||
    ...
    spec epilogue_gas_payer(
        ...
        aborts_if !exists<CoinStore<GasCoin>>(gas_payer);
        // Sufficiency of funds
        aborts_if !(global<CoinStore<GasCoin>>(gas_payer).coin.value >= transaction_fee_amount);

        aborts_if !exists<Account>(addr);
        aborts_if !(global<Account>(addr).sequence_number < MAX_U64);

        let pre_balance = global<coin::CoinStore<GasCoin>>(gas_payer).coin.value;
        let post balance = global<coin::CoinStore<GasCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);
        ensures balance == pre_balance - transaction_fee_amount;
        ensures account.sequence_number == pre_account.sequence_number + 1;
