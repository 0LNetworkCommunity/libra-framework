...
module ol_framework::test_donor_voice_governance {
    ...
    fun reauthorize_tx(framework: &signer, marlon_sponsor: &signer) {
      ...
      while (i < vector::length(&donor_sigs)) {
        ...
        let (_percent_approval, turnout_percent, _threshold_needed_to_pass, _epoch_deadline, _minimum_turnout_required) =  donor_voice_governance::get_reauth_tally(dv_address);
      ...
      let (percent_approval, turnout_percent, threshold_needed_to_pass, epoch_deadline, minimum_turnout_required) =  donor_voice_governance::get_reauth_tally(dv_address);
    ...
    fun reauthorize_poll_closes(framework: &signer, marlon_sponsor: &signer) {
      ...
      let (percent_approval, _turnout_percent, _threshold_needed_to_pass, epoch_deadline,_minimum_turnout_required) =  donor_voice_governance::get_reauth_tally(dv_address);
      ...
      diem_std::debug::print(&e);
    }

    #[test(framework = @ol_framework, marlon_sponsor = @0x1234)]
    fun reauthorize_duplicate_tx(framework: &signer, marlon_sponsor: &signer) {
      let _vals = mock::genesis_n_vals(framework, 3);
      mock::ol_initialize_coin_and_fund_vals(framework, 100000, true);
