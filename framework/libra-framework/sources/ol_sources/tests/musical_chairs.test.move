...
module ol_framework::test_musical_chairs {
    ...
    public entry fun eval_compliance_happy(root: signer) {
      ...
      let (good, bad, ratio) = musical_chairs::test_eval_compliance(&root, vals, 10);
      assert!(vector::length(&good) == 5, 7357002);
      assert!(vector::length(&bad) == 0, 7357003);
      assert!(fixed_point32::is_zero(ratio), 7357004);


      let (outgoing_compliant_set, new_set_size) = musical_chairs::test_stop(&root, 10); // epoch_round = 10
    ...
    public entry fun eval_compliance_one_val(root: signer) {
      ...
      let (good, bad, bad_ratio) = musical_chairs::test_eval_compliance(&root, vals, 10); //epoch_round = 10
      ...
      let (_outgoing_compliant_set, _new_set_size) = musical_chairs::test_stop(&root, 10); // epoch_round = 10
