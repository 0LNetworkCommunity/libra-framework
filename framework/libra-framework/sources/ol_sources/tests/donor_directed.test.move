...
module ol_framework::test_donor_directed {
  ...
  // use diem_std::debug::print;
    ...
    fun dd_init(root: &signer, alice: &signer) {
      ...
      donor_voice::make_donor_directed(&resource_sig, vals, 2);


      let list = donor_voice::get_root_registry();
      assert!(vector::length(&list) == 1, 7357001);

      assert!(donor_voice::is_donor_directed(donor_directed_address), 7357002);
    }
    ...
    fun dd_propose_payment(root: &signer, alice: &signer, bob: &signer) {
      ...
      let uid = donor_voice::propose_payment(bob, donor_directed_address, @0x1000b, 100, b"thanks bob");
      let (found, idx, status_enum, completed) = donor_voice::get_multisig_proposal_state(donor_directed_address, &uid);
      ...
      assert!(!donor_voice::is_scheduled(donor_directed_address, &uid), 7357008);
    }
    ...
    fun dd_schedule_happy(root: &signer, alice: &signer, bob: &signer, carol: &signer) {
      ...
      let uid = donor_voice::propose_payment(carol, donor_directed_address, @0x1000b, 100, b"thanks bob");
      let (found, idx, status_enum, completed) = donor_voice::get_multisig_proposal_state(donor_directed_address, &uid);
      assert!(found, 7357004);
      assert!(idx == 0, 7357005);
      assert!(status_enum == 1, 7357006);
      ...
      let list = donor_voice::find_by_deadline(donor_directed_address, 3);
    ...
    fun dd_propose_and_veto(root: &signer, alice: &signer, bob: &signer, carol: &signer, dave: &signer, eve: &signer) {
      ...
      let is_donor = donor_voice_governance::check_is_donor(donor_directed_address, signer::address_of(eve));
      assert!(is_donor, 7357002);

      // Dave will also be a donor
      ol_account::transfer(dave, donor_directed_address, 1);
      let is_donor = donor_voice_governance::check_is_donor(donor_directed_address, signer::address_of(dave));
      ...
      let uid_of_transfer = donor_voice::propose_payment(bob, donor_directed_address, @0x1000b, 100, b"thanks bob");

      let (_found, _idx, _status_enum, completed) = donor_voice::get_multisig_proposal_state(donor_directed_address, &uid_of_transfer);
      ...
      let has_veto = donor_voice_governance::tx_has_veto(donor_directed_address, guid::id_creation_num(&uid_of_transfer));
      ...
      let uid_of_transfer = donor_voice::propose_payment(carol, donor_directed_address, @0x1000b, 100, b"thanks bob");
      let (_found, _idx, state) = donor_voice::get_schedule_state(donor_directed_address, &uid_of_transfer);
      ...
      donor_voice::vote_veto_tx(eve, donor_directed_address, id_num);

      let (approve_pct, req_threshold) = donor_voice_governance::get_veto_tally(donor_directed_address, guid::id_creation_num(&uid_of_transfer));
      ...
      mock::trigger_epoch(root);
      donor_voice::vote_veto_tx(dave, donor_directed_address, id_num);

      // it is not yet scheduled, it's still only a proposal by an admin
      assert!(!donor_voice::is_scheduled(donor_directed_address, &uid_of_transfer), 7357010);
    ...
    fun dd_process_unit(root: &signer, alice: &signer, bob: &signer, carol: &signer) {
      ...
      donor_voice::process_donor_directed_accounts(root, 3);
    ...
    fun dd_process_epoch_boundary(root: &signer, alice: &signer, bob: &signer, carol: &signer, marlon_rando: &signer) {
      ...
      let uid = donor_voice::propose_payment(bob, donor_directed_address, signer::address_of(marlon_rando), 100, b"thanks marlon");
      let (found, idx, status_enum, completed) = donor_voice::get_multisig_proposal_state(donor_directed_address, &uid);
      assert!(found, 7357004);
      assert!(idx == 0, 7357005);
      assert!(status_enum == 1, 7357006);
      ...
      let uid = donor_voice::propose_payment(carol, donor_directed_address, signer::address_of(marlon_rando), 100, b"thanks marlon");
      let (found, idx, status_enum, completed) = donor_voice::get_multisig_proposal_state(donor_directed_address, &uid);
      assert!(found, 7357004);
      assert!(idx == 0, 7357005);
      assert!(status_enum == 1, 7357006);
    ...
    fun dd_liquidate_to_donor(root: &signer, alice: &signer, dave: &signer, eve: &signer) {
      ...
      donor_voice::propose_liquidation(dave, donor_directed_address);

      assert!(donor_voice_governance::is_liquidation_propsed(donor_directed_address), 7357004);

      // Dave proposed, now Dave and Eve need to vote
      donor_voice::vote_liquidation_tx(eve, donor_directed_address);

      mock::trigger_epoch(root);
      // needs a vote on the day after the threshold has passed
      donor_voice::vote_liquidation_tx(dave, donor_directed_address);

      let list = donor_voice::get_liquidation_queue();
      assert!(vector::length(&list) > 0, 7357005);

      let (addrs, refunds) = donor_voice::get_pro_rata(donor_directed_address);
