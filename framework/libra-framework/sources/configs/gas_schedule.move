...
module diem_framework::gas_schedule {
    use std::error;
    ...
    use diem_framework::util::from_bytes;
    use diem_framework::storage_gas::StorageGasConfig;
    ...
    public(friend) fun initialize(diem_framework: &signer, gas_schedule_blob: vector<u8>) {
        system_addresses::assert_diem_framework(diem_framework);
        assert!(!vector::is_empty(&gas_schedule_blob), error::invalid_argument(EINVALID_GAS_SCHEDULE));

        // TODO(Gas): check if gas schedule is consistent
        let gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
        move_to<GasScheduleV2>(diem_framework, gas_schedule);
    }

    /// This can be called by on-chain governance to update the gas schedule.
    public fun set_gas_schedule(diem_framework: &signer, gas_schedule_blob: vector<u8>) acquires GasSchedule, GasScheduleV2 {
        system_addresses::assert_diem_framework(diem_framework);
        assert!(!vector::is_empty(&gas_schedule_blob), error::invalid_argument(EINVALID_GAS_SCHEDULE));

        if (exists<GasScheduleV2>(@diem_framework)) {
            let gas_schedule = borrow_global_mut<GasScheduleV2>(@diem_framework);
            ...
            *gas_schedule = new_gas_schedule;
            ...
            if (exists<GasSchedule>(@diem_framework)) {
                _ = move_from<GasSchedule>(@diem_framework);
            };
            let new_gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
            // TODO(Gas): check if gas schedule is consistent
            move_to<GasScheduleV2>(diem_framework, new_gas_schedule);
        };
    ...
    public fun set_storage_gas_config(diem_framework: &signer, config: StorageGasConfig) {
        storage_gas::set_config(diem_framework, config);
