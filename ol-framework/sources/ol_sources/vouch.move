...
module ol_framework::vouch {
  ...
  #[test_only] use ol_framework::test_helpers;
  ...
  // val: validator
  #[test(val_1 = @0x11, val_2 = @0x22)]
  fun test_init(val_1: signer, val_2: signer) acquires Vouch {
    let vals: vector<signer> = vector[val_1, val_2];
    test_helpers::init_validators(&vals);
    let val_1 = vector::borrow(&vals, 0);
    let val_2 = vector::borrow(&vals, 1);
    let val_1_addr = signer::address_of(vector::borrow(&vals, 0));
    let val_2_addr = signer::address_of(vector::borrow(&vals, 1));

    init(val_1);
    assert!(is_init(val_1_addr), 7347001);
    vouch_for(val_2, val_1_addr);
    let includes = vector::contains(
      &get_buddies(val_1_addr), &val_2_addr
    );
    assert!(includes, 7357003);
    revoke(val_2, val_1_addr);
    let includes = vector::contains(&get_buddies(val_1_addr), &val_2_addr);
    assert!(!includes, 7357005);
  }

  // val: validator
  #[test(val_1 = @0x11, val_2 = @0x22, val_3 = @0x33)]
  fun test_init_genesis(val_1: signer, val_2: signer, val_3: signer) 
  // acquires Vouch 
  {
    let vals: vector<signer> = vector[val_1, val_2, val_3];
    test_helpers::init_validators(&vals);
    let _val_1_addr = signer::address_of(vector::borrow(&vals, 0));
    // let _val_2_addr = signer::address_of(&val_2);
